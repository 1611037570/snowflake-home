...
const basicInfoList = computed(() => {
  const list = []
  if (user.value.sex) list.push(user.value.sex)
  if (age.value) list.push(`${age.value}岁`)
  if (workYears.value) list.push(`${workYears.value}年经验`)
  return list
})
...
<template>
  ...
  <div class="flex items-center" :style="[lineHeightValue(22)]">
    ...
    <div class="ml-4 flex items-center gap-3" :style="[fontValue(2)]">
      <template v-for="(item, index) in basicInfoList" :key="index">
        <span>{{ item }}</span>
        <span v-if="index < basicInfoList.length - 1" class="h-3 w-px bg-current opacity-50"></span>
      </template>
    </div>
  </div>
  <!-- 联系方式 -->
