...
<template>
  <div class="flex w-full flex-col gap-1">
    ...
    <!-- 图片预览或上传入口：宽度随 size 百分比变化 -->
    <div
      class="group relative flex cursor-pointer items-center justify-center overflow-hidden rounded-lg border border-dashed border-sf-border text-sf-text-3 transition-colors hover:border-sf-theme hover:text-sf-theme"
      :style="{ width: `${size}%` }"
    ...
    <div class="flex items-center gap-3">
      <span class="w-12 shrink-0 text-xs text-sf-text-2">显示大小</span>
