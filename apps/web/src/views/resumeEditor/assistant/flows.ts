...
4. 本次不修改简历内容，只输出分析内容，不返回简历数据。
5. 直接在 Markdown 正文中输出英文翻译，用二级标题「问题回复」组织。`
          : `# 任务：将简历内容翻译成中文
请将用户的简历数据翻译成中文。
要求：
1. 逐项翻译所有字段内容，保持结构对应，不增删、不改写原意。
...
3. 严格基于简历真实数据，严禁编造任何信息。
4. 本次不修改简历内容，只输出分析内容，不返回简历数据。
5. 直接在 Markdown 正文中输出中文翻译，用二级标题「问题回复」组织。`,
...
3. 严格基于简历真实数据，严禁编造任何信息。
4. 本次不修改简历内容，只输出分析内容，不返回简历数据。
5. 直接在 Markdown 正文中输出自我介绍口头稿，用二级标题「问题回复」组织。`,
...
3. 严格基于简历真实数据，严禁编造任何信息。
4. 本次不修改简历内容，只输出分析内容，不返回简历数据。
5. 直接在 Markdown 正文中输出打招呼语，用二级标题「问题回复」组织。`,
...
请基于该 JD 对用户简历进行对标优化。
要求：
1. 先调用 load_resume_writing 获取简历编写流程，再按流程通过 propose_resume_edits 提交对标优化修改（update 指定 module，数组模块带 index），不得跳过工具调用。
2. 严格基于简历真实数据，严禁编造任何信息。
3. 在 Markdown 正文中用二级标题「修改说明」说明对标调整思路。`,
...
要求：
1. 先调用 load_resume_writing 获取简历编写流程，再按流程通过 propose_resume_edits 提交优化修改（update 指定 module，数组模块带 index），不得跳过工具调用。
2. 保持简历内容真实，不增删、不编造任何信息。
3. 在 Markdown 正文中用二级标题「修改说明」说明优化内容。`,
...
要求：
1. 确认目标模块已有内容，避免重复添加。
2. 先调用 load_resume_writing 获取简历编写流程，再按流程通过 propose_resume_edits 提交起草内容：数组型模块用 op: add 新增记录并携带 record，对象型模块用 update 写入字段；新增记录会同步出现在表单与预览中，不得跳过工具调用。
3. 起草内容围绕用户描述的模块与方向展开，突出成果与量化指标，使用专业表达；用户未提供的事实细节以通用示例占位，便于确认后修改为真实信息。
4. 在 Markdown 正文中用二级标题「修改说明」说明起草内容，并提醒用户核对、修改草稿后再用于简历。`,
...
5. 本次不修改简历内容，只输出分析内容，不返回简历数据。
6. 在 Markdown 正文中用二级标题「问题回复」输出本轮面试问题。`
          : `# 任务：AI 模拟面试（项目深挖方向）
...
4. 本次不修改简历内容，只输出分析内容，不返回简历数据。
5. 在 Markdown 正文中用二级标题「问题回复」输出打分结果与改进建议。`,
